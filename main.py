...
def view_lab_inventory() -> str:
    """Retrieves the complete production SQLite database inventory."""
    conn = sqlite3.connect(DB_FILE)
    cursor = conn.cursor()
...
def save_record(record_id: str, item_name: str, status: str, details: str) -> str:
    """Inserts or updates a record securely inside the production SQL table."""
    conn = sqlite3.connect(DB_FILE)
    cursor = conn.cursor()
...
def save_records_bulk(records_json: str) -> str:
    """Accepts multiple records and executes an atomic relational SQL transaction batch."""
    try:
