...
with st.sidebar:
    ...
    # 🔍 STEP 1: Target SKU Input (This triggers the lookup)
    sku_to_update = st.text_input("Target SKU / Record ID", placeholder="e.g. SKU101")
    
    # Initialize session state variables to safe defaults if they don't exist yet
    ...
    if sku_to_update and sku_to_update != st.session_state.last_checked_sku:
        ...
        if not matched_row.empty:
            ...
            # Reset values to zero if a completely new/unrecognized SKU is typed
        ...
        st.rerun() # Refresh layout instantly to push the values into the form fields below

    # 📝 STEP 3: The Update Form (Now powered by Memory Defaults)
    with st.form("inventory_update_form", clear_on_submit=True):
        
        # Pull values straight out of session_state memory
        ...
        new_notes = st.text_area("Update Warehouse Notes", value=st.session_state.form_notes, placeholder="Include location adjustments or alerts...")
            ...
            if not sku_to_update:
                ...
                with st.spinner("Synchronizing transaction metrics..."):
                    ...
                    if result == "success":
                        ...
                        # Clear memory cache on successful database mutation
