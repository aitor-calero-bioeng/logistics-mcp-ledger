...
# Force-install authlib manually inside the server environment at runtime
try:
    import authlib
except ImportError:
    subprocess.check_call([sys.executable, "-m", "pip", "install", "authlib>=1.3.2"])
    # Clear the module cache so Python recognizes the fresh install immediately
    import importlib
...
# 1️⃣ FIRST: Configure the page layout (Mandatory Streamlit Rule)
st.set_page_config(page_title="Enterprise Logistics Dashboard", page_icon="📦", layout="wide")

# 2️⃣ SECOND: Stop unauthorized users from running database queries
...
st.write(f"Bienvenido de nuevo, {st.user.name}!")
